...
(define-constant ERR-WITHDRAW-TIMELOCK (err u217)) ;; Revenue cannot be withdrawn yet
;; Platform fee percentage (2% of ticket price)
;; Calculated as: (price * 2) / 100
(define-constant PLATFORM-FEE-PERCENT u2)
;; Revenue timelock duration (in blocks)
 
(define-constant WITHDRAW-TIMELOCK u144)
...
(define-read-only (get-last-token-id)
  (let ((next-id (contract-call? .stackstix-storage get-next-ticket-id)))
...
(define-read-only (get-token-uri (token-id uint))
  (begin
    ...
    (match (contract-call? .stackstix-storage get-ticket token-id)
      ticket 
        ;; Ticket found, get event metadata
        (match (contract-call? .stackstix-storage get-event (get event-id ticket))
...
(define-read-only (get-owner (token-id uint))
  (begin
    ...
    (match (contract-call? .stackstix-storage get-ticket-owner token-id)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (let ((ticket (unwrap! (contract-call? .stackstix-storage get-ticket token-id) ERR-TICKET-NOT-FOUND)))
      ...
      (let ((event (unwrap! (contract-call? .stackstix-storage get-event (get event-id ticket)) ERR-EVENT-NOT-FOUND)))
        ...
        (try! (contract-call? .stackstix-storage update-ticket-owner token-id recipient))
...
(define-public (create-event
  ...
  (begin
    ...
    (let ((event-id (try! (contract-call? .stackstix-storage increment-event-id))))
      
      ;; Insert event data into storage
      ;; Organizer is set to tx-sender (caller becomes event owner)
      (try! (contract-call? .stackstix-storage insert-event
        event-id
        tx-sender           ;; Organizer
        name
        description
...
(define-public (update-event-metadata (event-id uint) (metadata-uri (optional (string-utf8 256))))
  (begin
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      (try! (contract-call? .stackstix-storage update-event-metadata event-id metadata-uri))
...
(define-public (cancel-event (event-id uint))
  (begin
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      (try! (contract-call? .stackstix-storage update-event-status event-id false))
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      (let ((sold-count (get sold-count event))
        ...
        (let ((ticket-price (get ticket-price event)))
          ...
          (let ((ticket-id (try! (contract-call? .stackstix-storage increment-ticket-id))))
            
            ;; Create ticket (mint NFT) in storage
            (try! (contract-call? .stackstix-storage insert-ticket
              ticket-id
              event-id
            ...
            (try! (contract-call? .stackstix-storage update-event-sold-count event-id (+ sold-count u1)))
            ...
            (let ((platform-fee (/ (* ticket-price PLATFORM-FEE-PERCENT) u100))
              ...
              (let ((current-balance (default-to 
                ...
                (contract-call? .stackstix-storage get-event-balance event-id))))
...
(define-public (validate-ticket (ticket-id uint))
  (begin
    ;; Get ticket details
    (let ((ticket (unwrap! (contract-call? .stackstix-storage get-ticket ticket-id) ERR-TICKET-NOT-FOUND)))
      ...
      (let ((event-id (get event-id ticket))
            (event (unwrap! (contract-call? .stackstix-storage get-event (get event-id ticket)) ERR-EVENT-NOT-FOUND)))
        
        ;; Verify caller is an authorized validator for this event
        (let ((validator-data (unwrap! (contract-call? .stackstix-storage get-validator event-id tx-sender) ERR-NOT-VALIDATOR)))
          (asserts! (get is-active validator-data) ERR-NOT-VALIDATOR)
        )
        
        ;; Check event timing: must be between start and end time
        (asserts! (>= stacks-block-height (get start-time event)) ERR-EVENT-NOT-STARTED)
        (asserts! (<= stacks-block-height (get end-time event)) ERR-EVENT-ENDED)
        
        ;; Mark ticket as used in storage (prevents re-entry)
        (try! (contract-call? .stackstix-storage mark-ticket-used ticket-id))
        
        ;; Increment validator's count for analytics
        (try! (contract-call? .stackstix-storage increment-validator-count event-id tx-sender))
...
(define-public (refund-ticket (ticket-id uint))
  (begin
    ...
    (let ((ticket (unwrap! (contract-call? .stackstix-storage get-ticket ticket-id) ERR-TICKET-NOT-FOUND)))
      ...
      (let ((event-id (get event-id ticket))
            (event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
        ...
        (let ((ticket-price (get ticket-price event)))
          ...
          (let ((organizer-refund (- ticket-price (/ (* ticket-price PLATFORM-FEE-PERCENT) u100))))
            ...
            (let ((current-balance (unwrap! (contract-call? .stackstix-storage get-event-balance event-id) ERR-EVENT-NOT-FOUND)))
            ...
            (try! (contract-call? .stackstix-storage delete-ticket ticket-id))
            
            ;; Decrease sold count
            (try! (contract-call? .stackstix-storage update-event-sold-count 
              event-id 
...
(define-public (add-validator (event-id uint) (validator principal))
  (begin
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      (try! (contract-call? .stackstix-storage set-validator event-id validator true))
...
(define-public (remove-validator (event-id uint) (validator principal))
  (begin
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      (try! (contract-call? .stackstix-storage set-validator event-id validator false))
...
(define-public (withdraw-event-revenue (event-id uint))
  (begin
    ...
    (let ((event (unwrap! (contract-call? .stackstix-storage get-event event-id) ERR-EVENT-NOT-FOUND)))
      ...
      ;; Enforce timelock: event must be ended + buffer period
      (asserts!
        (> stacks-block-height (+ (get end-time event) WITHDRAW-TIMELOCK))
        ERR-WITHDRAW-TIMELOCK
      )
      ...
      (let ((event-balance (unwrap! (contract-call? .stackstix-storage get-event-balance event-id) ERR-EVENT-NOT-FOUND)))
        ...
        (let ((withdrawable-amount (get locked-balance event-balance)))
          ...
          ;; Ensure something exists to withdraw
          ...
          ;; Transfer funds
          (try! (as-contract
            (stx-transfer? withdrawable-amount tx-sender (get organizer event))
          ))
          
          ;; Update withdrawn tracking
          (try! (contract-call? .stackstix-storage increment-withdrawn
            event-id
            withdrawable-amount
          ))
...
(define-read-only (get-event-details (event-id uint))
  (contract-call? .stackstix-storage get-event event-id)
)

;; Get ticket details
(define-read-only (get-ticket-details (ticket-id uint))
  (contract-call? .stackstix-storage get-ticket ticket-id)
)

;; Get tickets remaining for an event
(define-read-only (get-tickets-remaining (event-id uint))
  (match (contract-call? .stackstix-storage get-event event-id)
...
(define-read-only (is-event-live (event-id uint))
  (match (contract-call? .stackstix-storage get-event event-id)
    event (and 
      (get is-active event)
      (>= stacks-block-height (get start-time event))
      (<= stacks-block-height (get end-time event))
    )
    false
  )
)
...
(define-read-only (is-event-validator (event-id uint) (user principal))
  (match (contract-call? .stackstix-storage get-validator event-id user)
