...
(define-public (insert-event 
  ...
  (begin
    ...
    (asserts! (>= start-time stacks-block-height) ERR-EVENT-IN-PAST)  ;; Cannot create past events
    ...
    (map-set events
      { event-id: event-id }
      {
        ...
        created-at: stacks-block-height     ;; Record creation time
      }
    )
  ...
  (begin
    ...
    (let ((ticket (unwrap! (map-get? tickets { ticket-id: ticket-id }) ERR-TICKET-NOT-FOUND)))
      ...
      (map-set tickets
        ...
        (merge ticket { 
          is-used: true,
          used-time: (some stacks-block-height)  ;; Record when ticket was used
        })
      )
      (ok (print { event: "ticket-used", ticket-id: ticket-id, time: stacks-block-height }))
    )
  )
)
      ...
      (if (not (get is-used ticket))
        (begin
          (map-set tickets
            ...
            (merge ticket { 
              is-used: true,
              used-time: (some stacks-block-height)
            })
          )
          true
        )
...
(define-public (set-validator (event-id uint) (validator principal) (is-active bool))
  (begin
    ...
    (map-set event-validators
      ...
      { is-active: is-active, validated-count: u0, added-at: stacks-block-height }
    )
    (print { 
